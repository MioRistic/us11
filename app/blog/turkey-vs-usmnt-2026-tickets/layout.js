// app/news/turkey-vs-usmnt-2026-tickets/layout.js

import React from 'react';

export const metadata = {
  title: "Türkiye vs USMNT 2026 Tickets: Why This Final Group Game Is Worth Experiencing Live",
  description:
    "Turkey has already been eliminated, while the USMNT has secured qualification. Find out why this final Group D match on June 25 in Los Angeles is still worth attending and how to get tickets.",
  keywords: [
    "Türkiye vs USMNT",
    "USMNT vs Turkey 2026",
    "World Cup 2026 tickets",
    "USMNT tickets",
    "SoFi Stadium",
    "June 25 2026",
    "USMNT group stage",
  ],
  openGraph: {
    title: "Türkiye vs USMNT 2026: Why This Match Is Worth Experiencing Live",
    description:
      "Even though both teams have their fate decided, this final group stage match offers a unique atmosphere. Get tickets and be part of the USMNT’s last group game at the 2026 World Cup.",
    url: "https://us11fc.com/blog/turkey-vs-usmnt-2026-tickets",
    siteName: "US11",
    images: [
      {
        url: "https://i.ibb.co/23Dfwpp9/Untitled-design-8.png",
        width: 1200,
        height: 630,
        alt: "Türkiye vs USMNT 2026 World Cup",
      },
    ],
    locale: "en_US",
    type: "article",
  },
  twitter: {
    card: "summary_large_image",
    title: "Türkiye vs USMNT 2026 Tickets & Preview",
    description:
      "The USMNT’s final group stage match at the 2026 World Cup. Find out why it’s still worth attending and where to buy tickets.",
    images: ["https://i.ibb.co/23Dfwpp9/Untitled-design-8.png"],
  },
  alternates: {
    canonical: "https://us11fc.com/blog/turkey-vs-usmnt-2026-tickets",
  },
};

export default function TurkeyVsUSMNTLayout({ children }) {
  return <>{children}</>;
}