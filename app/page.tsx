...
const TrendingPlayers = () => {
  const players = [
    ...
    { name: 'Jordan Morris', image: 'https://assets.goal.com/images/v3/getty-2090010997/crop/MM5DENBQGA5DCNRQGE5G433XMU5DAORWHIYTAMA=/GettyImages-2090010997.jpg?auto=webp&format=pjpg&width=3840&quality=60', link: '/teams/seattle-sounders/morris' },
  ];

  return (
    <div className="max-w-6xl mx-auto py-12 px-4">
      {/* Helvetica Style Header */}
      <h2 className="text-5xl md:text-6xl font-black tracking-[-2.5px] text-center mb-10 text-[#020617] dark:text-white">
        Trending Players
      </h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-6">
        {players.map((player) => (
          <Link
            key={player.name}
            href={player.link}
            className="group relative bg-white dark:bg-zinc-900 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2"
          >
            ...
            {/* Player Name - Helvetica Style */}
            <div className="p-4 text-center">
              <h3 className="text-xl md:text-2xl font-black tracking-[-1.5px] text-[#020617] dark:text-white leading-tight">
                {player.name}
              </h3>
            </div>
          </Link>
      ...
      <div className="px-7 py-8 lg:py-12 bg-[#0a0a0a] flex flex-col justify-center min-h-[240px] sm:min-h-[260px] md:min-h-[280px] lg:min-h-[340px]">
        ...
        >
          ...
          <button
            ...
            className="cursor-pointer bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed active:scale-95 transition-all font-bold text-white px-8 py-3.5 rounded-2xl text-sm tracking-wider whitespace-nowrap"
          >
            {formState.submitting ? 'Submitting...' : 'Subscribe to Newsletter'}
          </button>
        </form>
        ...
        {formState.succeeded && (
          <div className="mt-4 text-green-400 text-sm font-medium">
            ✅ Thank you for joining us! Check your email for confirmation.
          </div>
        )}
